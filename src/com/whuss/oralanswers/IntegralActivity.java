...
//	final UMSocialService mController = UMServiceFactory.getUMSocialService("com.umeng.share");
	...
	//上传自己的积分
	private String urlUploadScore = LoginActivity.URL + "index.php?option=com_content&postintegral=on"; 
	//积分排行，前7名数据
	private String urlRankTotal = LoginActivity.URL + "index.php?option=com_content&getIntegralList=on"; 
	//自己的排名
	private String urlRankSelf = LoginActivity.URL + "index.php?option=com_content&rankings=on"; 
	...
	private void getRankTotal() {
		...
		new Thread(){
			public void run() {
				...
				try {
					//拉取排名以及前7名的名字和积分
					data = DataManeger.getTestData(urlRankTotal+"&lang=en");
			...
			public void run() {
				...
				try {
					...
					//上传积分，返回积分是否成功，成功之后拉取排名以及前7名
						data = DataManeger.getTestData_TianQi(urlUploadScore+"&lang=en");
	...
	private void getRankSelf() {
		...
		new Thread(){
			public void run() {
				...
				try {
					//得到自己的排名
					data = DataManeger.getTestData_TianQi(urlRankSelf+"&lang=en");
					Test_Model_TianQi  test_Model_User = data.getData();
