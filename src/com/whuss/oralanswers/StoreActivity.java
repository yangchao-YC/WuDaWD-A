...
public class StoreActivity extends Activity implements android.view.View.OnClickListener {
	...
	//商品列表的url
	String urlStoreList = LoginActivity.URL + "index.php?option=com_content&list=on";
	
	//自己的积分排名url
	String urlScoreSelf = LoginActivity.URL + "index.php?option=com_content&rankings=on";
	...
	private void getUserScore(final String username) {
		...
		new Thread(){
			public void run() {
				...
				try {
					//得到自己的积分
					urlScoreSelf = urlScoreSelf + "&username="+username+"&lang=en";
	...
	private void dataThread() {
		...
		new Thread(){
			public void run() {
				Test_Bean data;
				try {
					data = DataManeger.getTestData(urlStoreList+"&lang=en");
